                ...
                new Response.Listener<JSONArray>() {
                    ...
                    public void onResponse(JSONArray response) {
                        try {
                            ...
                            //create today's date
                            //Date date = new Date();
                            String selected_date = formatter.format(date);
                            System.out.println("Selected date is: " + selected_date);
                            ...
                            for (int i = 0; i < daySchedule.size(); i++) {
                                ...
                                for (int j = 0; j < response.length(); j++) {
                                    ...
                                    if (startDate.equals(selected_date)) {
                            ...
                            for (int i = 0; i < daySchedule.size() - 1; i++) {
                                if (daySchedule.get(i).equals(freeTimesList.get(i)) && daySchedule.get(i + 1).equals(freeTimesList.get(i + 1))) {
                                    ...
                                    fillerReservation.setStartDate(selected_date);
                                    reservations.add(fillerReservation);
                                    //skips ahead by one
                                    i++;
                                } else {
                                    for (int j = 0; j < response.length(); j++) {
                                        //check for unique reservation
                                            JSONObject JSONreservation = response.getJSONObject(j);
                                            ...
                                            if (selected_date.equals(startDate)) {
                                                if (daySchedule.get(i).equals(startTime)) {
                                                    ...
                                                    boolean hasEnteredFirstIf = false;
                                                    while (!resEndTimeReached) {
                                                        if (!daySchedule.get(i + 1).equals(endTime) && !daySchedule.get(i + 2).equals(endTime)) {
                                                            ...
                                                            hasEnteredFirstIf = true;
                                                            i += 2;
                                                        } else {
                                                            resEndTimeReached = true;
                                                            if(hasEnteredFirstIf) {
                                                                i--;
                                                            }else{
                                                                i++;
                                                            }
